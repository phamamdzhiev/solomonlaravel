        ...
        <form>
            ...
            <div class=" align-center">
                <div>
                    ...
                    <input type="text" class=" rounded px-2" id="vet" value="{{ $data->vet }}" name="vet"
                        readonly>
                </div>
                <div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr">
                <div>
                    ...
                    <input type="text" class=" rounded px-2" name="no" value="{{ $data->no }}" readonly
                        id="no">
                    <label for="city">Населено място*</label>
                    <input type="text" class=" rounded px-2" name="city" value="{{ $data->city }}" readonly
                        id="city">
                </div>
                <div>
                    <label for="names">Име на собственика или фирма*</label>
                    <input type="text" class=" rounded px-2" name="names" value="{{ $data->names }}" readonly
                        id="names">
                    <label for="egn">ЕГН*</label>
                    <input type="text" class=" rounded px-2" name="egn" value="{{ $data->egn }}" readonly
                        id="egn">
                </div>
                <div class="">
                ...
                <div class="">
                    ...
                    <textarea name="invoice" class=" rounded px-2 w-full" id="invoice" readonly cols="30" rows="2">{{ $data->invoice ?? '---' }}</textarea>
                </div>

                <div class="">
                    <label for="marka_model">Модел марка</label>
                    <input type="text" class=" rounded px-2" value="{{ $data->marka_model ?? 'Не е посочено' }}"
                        id="marka_model">
                </div>
            ...
            <div>
                ...
                <table class="-spacing-0.5 text-center w-full" id="formlazer">
                    ...
                    <tbody>
                        @foreach (json_decode($data->field1) as $key => $f1)
                            ...
                            @endif
                            <tr>
                                <td class=""><input readonly value="{{ json_decode($data['field1'])[$key] }}"
                                        type="text" class="w-full text-center" /></td>
                                <td class=""><input readonly value="{{ json_decode($data['field2'])[$key] }}"
                                        type="text" class="w-full text-center" />
                                </td>
                                <td class=""><input readonly value="{{ json_decode($data['field3'])[$key] }}"
                                        type="text" class="w-full text-center" />
                                </td>
                            </tr>
                        @endforeach
                ...
                <p class="my-1">Дата на запознаване и съгласие с Условията за защита на личните данни
                    <strong>{{ date('Y-m-d H:i:s', strtotime('+3 hours')) }}</strong>
                </p>
